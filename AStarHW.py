...
class astar_search:
    ...
    def search(self):
        ...
        self.open[self.srcState] = 0 + self.cal_hn(self.srcState)

        while len(self.open) > 0:
            curState = min(self.open, key=self.open.get)  #找到fn最小的状态 返回的是键名
            del self.open[curState] #移除
            self.close.append(curState)
            ...
            for nShift in lst_shifts:
                ...
                if (newState in self.close) == False:  # 判断交换后的状态是否在close表中
                    if self.open.get(newState) == None:
                        self.g_dict_states_gn[newState] = self.g_dict_states_gn[curState] + 1 #存入深度
                        self.open[newState] = fn
                        self.path[newState] = curState  # g_dict_states[子序列] = 父序列 定义前驱结点
                    else:
                        if fn < self.open[newState]:
                            self.open[newState] = fn
                            self.path[newState] = curState
                            self.g_dict_states_gn[newState] = self.g_dict_states_gn[curState] + 1
        lst_steps = []
    ...
    if retCode != 0:
        ...
        for nIndex in range(len(lst_steps)):
            ...
            if nIndex != len(lst_steps) - 1:
                print('->')
    print("A*算法花费时间：" + str(end_d - now_d) + "s")